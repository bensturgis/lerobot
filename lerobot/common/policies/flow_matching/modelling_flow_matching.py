...
class FlowMatchingPolicy(PreTrainedPolicy):
    ...
    def __init__(
        ...
        if config.sample_with_uncertainty and config.uncertainty_sampler == "cross_likelihood":
            scorer_config = copy.deepcopy(config)
            scorer_config.sample_with_uncertainty = False
            self.scorer = FlowMatchingModel(scorer_config)
        else:
            self.scorer = None
    ...
    def _init_uncertainty_sampler(self):
        """
        Constructs the uncertainty sampler based on the config.
        """
        if not self.config.sample_with_uncertainty:
            self.uncertainty_sampler = None
            return

        scorer_unet = self.scorer.unet if self.scorer is not None else None
        self.uncertainty_sampler = make_flow_matching_uncertainty_sampler(
            cfg=self.config,
            velocity_model=self.flow_matching.unet,
            scorer_velocity_model=scorer_unet,
        )
    ...
    def generate_actions(self, batch: dict[str, Tensor]) -> Tensor:
        ...
        if not self.training and self.config.sample_with_uncertainty:
            ...
            if self.config.uncertainty_sampler == "cross_likelihood":
            ...
            if self.config.uncertainty_sampler == "composed_likelihood":
