...
class FlowMatchingPolicy(PreTrainedPolicy):
    ...
    def __init__(
        ...
        self.fiper_data_recorder_config = fiper_data_recorder_config
        ...
        if (
            (uncertainty_sampler_config is not None and getattr(uncertainty_sampler_config.active_config, "scorer_type", None) == "ensemble") or
            fiper_data_recorder_config is not None
        ):
    ...
    def init_uncertainty_sampler(
        ...
        self.uncertainty_sampler = make_flow_matching_uncertainty_sampler(
            ...
            ensemble_flow_matching_model=self.scorer,
            laplace_calib_loader=laplace_calib_loader,
            laplace_path=laplace_path,
        )

    def init_fiper_data_recorder(self):
        """
        ...
        if self.fiper_data_recorder_config is None:
            return

        if self.scorer is None:
            raise ValueError(
                "FIPER data recorder requires a scorer model to compute log-likelihoods of actions."
            )

        self.fiper_data_recorder = FiperDataRecorder(
            config=self.fiper_data_recorder_config,
