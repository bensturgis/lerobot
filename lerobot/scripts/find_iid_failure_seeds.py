...
def rollout(
    ...
    The return dictionary contains:
        ...
        "seeds": If provided, the seeds used to reset each of the environments.
...
def eval_policy(
    ...
    for batch_ix in progbar:
        ...
        if max_episodes_rendered > 0 and len(ep_frames) > 0:
            ...
            for batch_idx, (stacked_frames, done_index) in enumerate(
                zip(
                ...
                if batch_successes[batch_idx]:
                    continue

                videos_dir.mkdir(parents=True, exist_ok=True)
                video_path = videos_dir / f"eval_seed_{seeds[batch_idx]}.mp4"
    ...
    info = {
        "per_episode": [
            ...
            for (sum_reward, max_reward, success, seed) in zip(
...
def main(cfg: FindIidFailureSeedsConfig):
    ...
    while len(iid_failure_seeds) < cfg.num_iid_failure_seeds:
        ...
        if eval_info_path.exists():
            with eval_info_path.open("r") as f:
                eval_info = json.load(f)
            eval_info["per_episode"].append(cur_eval_info["per_episode"])
        else:
            eval_info = cur_eval_info
